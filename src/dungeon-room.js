...
export class DungeonRoom {
    ...
    load() {
        ...
        this.game.physics.add.collider(this.game.player.gameObject, borderTiles);
    ...
    update(time, delta) {
        ...
        if (constants.ROOM_W / 2 - constants.TILE_SIZE/2 < px && px < constants.ROOM_W / 2 + constants.TILE_SIZE/2) {
            if (py > constants.ROOM_H - constants.TILE_SIZE && player.gameObject.body.velocity.y > 1) {
                this.game.changeRoom(this.x, this.y + 1);
            }
            else if (py < constants.TILE_SIZE && player.gameObject.body.velocity.y < -1) {
                this.game.changeRoom(this.x, this.y - 1);
            }
        }
        else if (constants.ROOM_H / 2 - constants.TILE_SIZE/2 < py && py < constants.ROOM_H / 2 + constants.TILE_SIZE/2) {
            if (px > constants.ROOM_W - constants.TILE_SIZE && player.gameObject.body.velocity.x > 1) {
                this.game.changeRoom(this.x + 1, this.y);
            }
            else if (px < constants.TILE_SIZE && player.gameObject.body.velocity.x < -1) {
                this.game.changeRoom(this.x - 1, this.y);
            }
        }
    }
